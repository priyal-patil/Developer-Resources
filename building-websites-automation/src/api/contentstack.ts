 ...
 * 30+ minutes). A process-lifetime cache meant teardown deletes came back
 * 401, the failure was swallowed by an upstream `.catch(() => false)`, and
 * the stack was orphaned in the shared QA org — same root cause fixed in
 * kickstart-automation's client. Pass `forceRefresh` anywhere reliability
 * beats saving one login round-trip — teardown, in particular.
 */
...
export async function listRunStacks(): Promise<{ apiKey: string; name: string; createdAt: string }[]> {
  ...
  if (!res.ok) return [];
  const data = await res.json();
  // Matches both this project's default fallback name ("CS Kickstart <name>",
  // shared pipeline code with kickstart-automation) and any doc-specific
  // override starting with "BW " (e.g. config/docs.json's stackName field).
  return (data.stacks ?? [])
    .filter((s: any) => /^(CS Kickstart |BW )/.test(s.name ?? ""))
...
// Allow: `tsx src/api/contentstack.ts delete <apiKey> [<apiKey>...]`  /  `tsx src/api/contentstack.ts sweep [<maxAgeHours>]`
if (process.argv[1]?.endsWith("contentstack.ts")) {
  if (process.argv[2] === "delete") {
    ...
    console.log(`Sweeping "CS Kickstart *" / "BW *" stacks older than ${hours}h…`);
