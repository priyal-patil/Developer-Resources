    ...
    try {
      ...
      for (const route of cfg.navRoutes ?? []) {
        try {
          ...
          // Header+footer alone ≈ the chrome the app always renders; a route with no
          // entry has no content beyond that.
          const hasContent = text.length > 450;
          results.push({
            step: verifyStep(`Nav route ${route}`),
            status: hasContent ? "passed" : "failed",
            detail: hasContent
              ? `${route} renders content (${text.length} chars)`
              : `GAP EVIDENCE: ${route} renders no page content beyond the header/footer chrome (${text.length} chars) — the doc's navigation links here but never creates the entries for it (see screenshot)`,
            evidence: shotPath,
          });
        } catch (e) {
          results.push({ step: verifyStep(`Nav route ${route}`), status: "failed", detail: `${route} failed to render: ${(e as Error).message}` });
        }
      }
    } finally {
      await browser.close();
